#pragma once
#include <array>
#include <cstdint>
#include <iostream>
#include <string>
#include <utility>
#include <vector>
#include <algorithm>

using u64 = uint64_t;
using u32 = uint32_t;
using std::array;
using std::cout;
using std::endl;
using std::pair;
using std::string;
using std::string_view;
using std::vector;

inline constexpr float INF = 1000000.0f;

enum class COLOUR { BLACK, WHITE, NONE };

enum class PIECE { EMPTY, PAWN, ROOK, KNIGHT, BISHOP, KING, QUEEN };

enum class FLAGS : u32 {
    // castling rights, set to 0 if king or any rooks have moved, or already castled. Default 1.
    CASTLE_KSIDE_WHITE = 1 << 0,
    CASTLE_QSIDE_WHITE = 1 << 1,
    CASTLE_KSIDE_BLACK = 1 << 2,
    CASTLE_QSIDE_BLACK = 1 << 3,

};

inline int lsb(const u64 &num) { return __builtin_ctzll(num); }
inline int msb(const u64 &num) { return 63 - __builtin_clzll(num); }
inline int pop_ct(const u64 &num) { return __builtin_popcountll(num); }
inline constexpr array<string_view, 3> colour_names = {"BLACK", "WHITE", "NONE"};
inline constexpr array<string_view, 7> piece_names = {"EMPTY", "PAWN", "ROOK", "KNIGHT", "BISHOP", "KING", "QUEEN"};
inline constexpr array<pair<int, int>, 8> knight_offsets = {pair{1, 2}, pair{1, -2}, pair{-1, 2}, pair{-1, -2},
                                                            pair{2, 1}, pair{2, -1}, pair{-2, 1}, pair{-2, -1}};
inline constexpr array<pair<int, int>, 8> king_offsets = {pair{1, 1}, pair{1, -1}, pair{-1, 1}, pair{-1, -1},
                                                          pair{0, 1}, pair{0, -1}, pair{1, 0},  pair{-1, 0}};